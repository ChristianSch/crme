...
function openTaskEntity(task: Todo, people: Person[], companies: Company[], deals: Deal[], onSelectPerson: (person: Person) => void, onSelectCompany: (company: Company) => void, onSelectDeal: (deal: Deal) => void) { if (task.entity_type === "person") { const person = people.find((item) => item.id === task.entity_id); if (person) onSelectPerson(person); } if (task.entity_type === "company") { const company = companies.find((item) => item.id === task.entity_id); if (company) onSelectCompany(company); } if (task.entity_type === "deal") { const deal = deals.find((item) => item.id === task.entity_id); if (deal) onSelectDeal(deal); } }
